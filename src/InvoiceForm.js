...
class InvoiceForm extends React.Component {
  ...
  return (
		...
		<tbody>
    <tr>
    <td><img className='photo' src='../no-image.png'/></td>
    <td><div className='center_name'>Web Code Education</div>
              <div className='top_txt'>
              <p>Power by Web Code Services <span className='custmargin'></span> Branch Code: WCEKKR01</p>
...
<div className="student_details">
	<b>Name:</b>{this.state.student_data.first_name} {this.state.student_data.last_name}<br/>
...
<b>Receipt Number:</b> #WCE/{this.state.student_data.id}/570162<br/>
...
<b>Total Fee:</b> &#8377;{this.state.total_fees}/-</div></td>
		...
		<tbody>
		{
			this.state.feesdata.map((item,index)=>{ count+= Number(item.fees);this.setState({total_sum:count})
					return (<tr key={index}>
					<td>{index+1}</td><td>{item.submit_date}</td><td>#WCE/K0101/20220816/1</td><td>1</td><td>&#8377; {item.fees}</td><td>&#8377; {item.fees}</td>
					</tr>)
			})
		}
		<tr>
		...
		<tr>
                            <td colSpan={6}>
                            	...
                            	<ol>
									...
									<li>3. Fees are not transferrable to any student in any case.</li>
                            	</ol>
                            </td>
